...
/**
 * RestoreMarker — 替代 React 的 features/ux/RestoreMarker.jsx
 * 在上次阅读位置注入标记气泡，方便用户跳回。
 */
...
let backFab: HTMLElement | null = null
let scrollHandler: (() => void) | null = null
let initTimer: ReturnType<typeof setTimeout> | null = null

onMounted(() => {
  const path = route.path
  const key = 'scroll_' + path.replace(/[^a-zA-Z0-9]/g, '_').substring(0, 40)
  ...
  initTimer = setTimeout(() => {
    ...
    el.innerHTML =
      '<div style="position:relative;display:flex;align-items:center;gap:12px;padding:12px 16px 12px 14px;background:var(--card,#1e2235);border:1px solid var(--border,#2d3436);border-left:3px solid var(--accent,#6c63ff);border-radius:12px;box-shadow:0 4px 20px rgba(0,0,0,.25);min-width:200px">' +
    ...
    back.style.cssText =
      'position:fixed;bottom:250px;right:30px;z-index:99;display:none;pointer-events:auto'
    back.innerHTML =
      '<div style="display:flex;align-items:center;gap:8px;padding:8px 14px 8px 12px;background:var(--card,#1e2235);border:1px solid var(--border,#2d3436);border-left:3px solid var(--accent,#6c63ff);border-radius:20px;box-shadow:0 4px 16px rgba(0,0,0,.25);cursor:pointer;font-size:.8rem;color:var(--accent,#6c63ff);font-weight:600;white-space:nowrap"><span>📍</span><span>上次学到这里</span></div>'
    document.body.appendChild(back)
    back.addEventListener('click', () => window.scrollTo({ top: saved - 80, behavior: 'smooth' }))
    backFab = back

    scrollHandler = () => {
      if (back) back.style.display = Math.abs(window.scrollY - saved) > 600 ? 'block' : 'none'
    }
